...
let recordBtn = document.getElementById("recordBtn"); // Botón de iniciar grabación
let stopBtn = document.getElementById("stopBtn"); // Botón de detener grabación
let timer = document.getElementById("timer"); // Elemento de tiempo transcurrido
let generateBtn = document.getElementById("generateBtn"); // Botón de generar
let improveBtn = document.getElementById("improveBtn"); // Botón de mejorar

// Claves de acceso a las APIs
let chatgptKey = "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"; // Clave de acceso a la API de ChatGPT
...
if (typeof recordAudio === 'function') {
    ...
    console.log("La función recordAudio() se está ejecutando.");
  } else {
...
// Función para limpiar los elementos de salida cuando se cambia el texto de la entrada
function clearOutput() {
    if (summary) {
      summary.textContent = ""; // Limpiar el elemento de salida de resumen
    }
    if (labels) {
      labels.textContent = ""; // Limpiar el elemento de salida de etiquetas
    }
    if (improvedSummary) {
      improvedSummary.textContent = ""; // Limpiar el elemento de salida de resumen mejorado
    }
    if (improvedLabels) {
      improvedLabels.textContent = ""; // Limpiar el elemento de salida de etiquetas mejoradas
    }
  }

  // Agregar la función de limpiar los elementos de salida al evento de cambio del elemento de entrada
input.addEventListener("change", clearOutput);

recordBtn.addEventListener("click", recordAudio);
stopBtn.addEventListener("click", stopAudio);
// Agregar la función de generar el resumen y las etiquetas al evento de clic del botón de generar
generateBtn.addEventListener("click", generateText);

// Agregar la función de mejorar el resumen y las etiquetas al evento de clic del botón de mejorar
